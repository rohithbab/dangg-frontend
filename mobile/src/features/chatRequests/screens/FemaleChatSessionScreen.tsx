...
function FemaleChatSessionScreen(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    // On (re)entry she is present — proactively clear any stale "stepped away"
    // marker (and pending reminders) left by an earlier background / force-close.
    // The listener below only fires on TRANSITIONS, so a fresh mount after a
    // force-close+reopen would otherwise leave the marker set and the male would
    // end the chat while she is actually here (#5).
    ...
    const sub = AppState.addEventListener('change', next => {
      if (next === 'background' && !pickingMediaRef.current) {
        ...
        // She's backgrounded and can't see the in-app banner — remind her via
