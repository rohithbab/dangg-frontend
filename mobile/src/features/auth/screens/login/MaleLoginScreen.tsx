...
type Nav = NativeStackNavigationProp<AuthStackParamList, 'MaleLogin'>;

const maleLoginSchema = z.object({
  phone: ZodSchemas.phoneIndian,
  password: z.string().min(1, 'Enter your password'),
});
type MaleLoginInput = z.infer<typeof maleLoginSchema>;

/**
 * Male login — single screen with both phone and password since males
 * have no verification gate.
 */
function MaleLoginScreen(): React.ReactElement {
  ...
  } = useForm<MaleLoginInput>({
    resolver: zodResolver(maleLoginSchema),
    mode: 'onChange',
    defaultValues: { phone: '', password: '' },
  });

  const onSubmit = useCallback(async (data: MaleLoginInput): Promise<void> => {
    setInlineError(null);
    ...
    try {
      ...
      if (e instanceof AppException) {
        ...
        logger.error('MaleLoginScreen.signIn failed', e);
      ...
      >
          ...
          <Card padding={AppSpacing.lg} containerStyle={styles.card}>
            ...
            <View style={styles.forgotRow}>
              <TextButton
                ...
                onPress={() => navigation.navigate('ForgotPasswordPhone', { role: UserRole.Male })}
              />
            </View>
          </Card>
        </ScrollView>
        <View style={styles.footer}>
          ...
          <View style={styles.signupRow}>
            ...
            <Pressable onPress={() => navigation.navigate('MaleSignupBasicInfo')} hitSlop={8}>
...
export default MaleLoginScreen;
